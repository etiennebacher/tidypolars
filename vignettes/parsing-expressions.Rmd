...
`var`, `arccos`, ...), string manipulation (`n_chars`, `starts`, ...), and date-time 
...
these are evaluated before they are wrap into `pl$lit()`.
...
is converted to `std(x, na.rm = TRUE)`. Since R and Polars functions don't share
the same name, this check relies on a custom list containing all equivalencies 
between R and Polars functions. You can see the list of supported R functions at
the bottom of this vignette.

Here, replacing R's `sd()` functions with `std()` is not enough because the 
argument `x` (which usually is a column of the dataset) is not in the right 
...
so if you want to check if your function returns a Polars expression, you have 
to provide a `pl$col()` call:
...
columns. It also accepts built-in functions and UDFs.
...
However, it will also error if the UDF you pass doesn't return a Polars expression.
Similarly, if you want to pass an anonymous function, it has to return a Polars 
expression:
