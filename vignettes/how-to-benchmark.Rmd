...
Using LazyFrames means that you also need to collect results in order to trigger the computations.
...
`compute()` will be faster because it doesn't go through the extra step of converting a Polars DataFrame to R, but it also means that one cannot pass the output of `compute()` directly to `ggplot2` to make a plot for instance.
...
It is also faster than the default engine in many cases, and not only on larger-than-RAM data.

**Note:** as of August 2025, the streaming engine is not used by default yet but it should become the default one in the next few months.
...
However, `tidypolars` shouldn't be used in such tools.
...
In [some cases](https://pola-rs.github.io/r-polars/vignettes/custom-functions.html#using-purrr), you may want to use those frameworks on Polars objects, but this should be the exception more than the norm.
