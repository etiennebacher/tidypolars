...
out <- tribble(
  ...
  mutate(
    Notes = case_when(
      ...
      Package == "`base`" & Function == "is.na" ~ "In R, `is.na(NaN)` returns `TRUE`, but this returns `FALSE` in tidypolars.",
      ...
      Package == "`base`" & Function == "`strptime`" ~
        "`strptime()` in base R returns a `POSIXlt` vector while `polars` converts the output to `POSIXct`. Use `as.POSIXct()` on the base R output to get the same result.",
      ...
      Package == "`dplyr`" & Function %in% c("`first`", "`last`", "`nth`") ~
        "The `default` argument uses Polars type coercion rules, which may change the output type instead of rejecting incompatible values as dplyr does.",
      ...
      Package == "`base`" & Function == "`as.integer`" ~
        "`tidypolars` errors if it cannot convert values to integers. This differs from `base` which returns `NA` in this case. This is to avoid silent differences when converting `'1.5'` for instance.",
      .default = ""
    )
  ) |>
  arrange(Package)
