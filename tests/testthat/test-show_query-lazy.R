...
test_that("sequences using group sizes can be replayed", {
  test_pl <- tibble(g = c("a", "b", "a")) |> as_polars_lf()
  query <- test_pl |>
...
test_that("check query for make_unique_id()", {
  ...
  test_pl <- as_polars_lf(data.frame(x = c("a", "b"), y = c(1, 2)))
...
test_that("check query for scan_*_polars() and read_*_polars()", {
  ...
  query <- scan_parquet_polars(dest) |>
    filter(cyl > 4) |>
    select(mpg)
  expect_snapshot_lazy(show_query(query), transform = redact_source)
