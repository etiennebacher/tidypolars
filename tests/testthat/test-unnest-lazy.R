...
test_that("values_to template works with multiple columns", {
  ...
  expect_equal_lazy(as.data.frame(result)$y_val, c(1L, 2L, 3L, 4L))
  expect_equal_lazy(as.data.frame(result)$z_val, c(5L, 6L, 7L, 8L))
})
...
test_that("indices_to template works with multiple columns", {
  ...
  expect_equal_lazy(as.data.frame(result)$y_idx, c(1L, 2L, 1L, 2L))
  expect_equal_lazy(as.data.frame(result)$z_idx, c(1L, 2L, 1L, 2L))
})
...
test_that("values_to template with dash separator works", {
  ...
  expect_equal_lazy(as.data.frame(result)$`y-n`, c(1L, 2L, 3L, 4L))
  expect_equal_lazy(as.data.frame(result)$`z-n`, c(5L, 6L, 7L, 8L))
})
