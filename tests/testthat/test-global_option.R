test_that("tidypolars_unknown_args: basic behavior works", {
  test_pl <- pl$DataFrame(x = c(2, 1, 5, 3, 1))

  expect_warning(
    test_pl |> mutate(x2 = sample(x, prob = 0.5)),
...
test_that("tidypolars_fallback_to_r: basic behavior works", {
  test_df <- tibble(x = c(2, 1, 5, 3, 1))
  test_pl <- as_polars_df(test_df)

  expect_error(
    test_pl |> mutate(x2 = mad(x)),
  ...
  withr::with_options(
    list(tidypolars_fallback_to_r = TRUE),
    {
      expect_equal(
        test_pl |> mutate(x2 = mad(x)),
        test_df |> mutate(x2 = mad(x))
      )

      # Can create other variables before and after
      expect_equal(
        test_pl |> mutate(y = 1, x2 = mad(x), z = 2),
        test_df |> mutate(y = 1, x2 = mad(x), z = 2)
      )
