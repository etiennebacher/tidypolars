### [GENERATED AUTOMATICALLY] Update test-bind_rows.R instead.
...
test_that("error if not all elements don't have the same class", {
  ...
  expect_snapshot_lazy(bind_rows_polars(p1, p2), error = TRUE)
})
...
test_that("basic behavior works", {
  ...
  expect_is_tidypolars(bind_rows(l))
...
test_that("arg .id works", {
  ...
  expect_equal_lazy(
    bind_rows(p1, p2, .id = "foo") |>
      pull(foo),
    as.character(rep(1:2, each = 20))
  )

  expect_equal_lazy(
    bind_rows(p1 = p1, p2 = p2, .id = "foo") |>
      pull(foo),
    rep(c("p1", "p2"), each = 20)
  )

  expect_equal_lazy(
    bind_rows(list(p1 = p1, p2 = p2), .id = "foo") |>
      pull(foo),
    rep(c("p1", "p2"), each = 20)
  )

  expect_equal_lazy(
    bind_rows(p1 = p1, p2, .id = "foo") |>
      pull(foo),
    as.character(rep(1:2, each = 20))
  )
})
...
test_that("error if not all elements don't have the same class", {
  ...
  expect_snapshot_lazy(bind_rows(p1, p2), error = TRUE)
})
