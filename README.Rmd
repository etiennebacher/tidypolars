...
that `tidypolars` also can't be on CRAN. That said, there are still several ways
to install `tidypolars`. Depending on your OS, the procedure is slightly 
different.
...
Since most of the work is simply rewriting `tidyverse` code into `polars` 
syntax, `tidypolars` is extremely close to `polars` performance.
