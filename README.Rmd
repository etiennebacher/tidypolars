...
`tidypolars` is built on `polars`, which is not available on CRAN for now. This
means that `tidypolars` can't be on CRAN for now. 

That said, you can install `tidypolars` with `remotes` or `pak`. Depending on
