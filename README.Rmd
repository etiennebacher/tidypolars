...
* `dbplyr`: allows using `dplyr` for data stored in a relational database, but doesn't provide specific optimizations [TO CHECK]
  - **How is tidypolars different?**: `tidypolars` runs a lot of optimizations when using lazy evaluation.
