...
pivot_wider.polars_data_frame <- function(
  ...
  # Need to maintain order because this is used to generate the column order
  # of pivoted columns.
  on_columns <- data$select(!!!names_vars)$unique(maintain_order = TRUE) |>
    as.data.frame()

  # TODO: if there are multiple columns in `on_columns` then it is possible that
  # the value combinations that create the new names still is duplicate with
  # an existing column name.
  if (ncol(on_columns) == 1) {
    overlapping_cols <- intersect(on_columns[[1]], data_names)
    if (length(overlapping_cols) > 0) {
      cli_abort(
        "This operation would generate column name(s) that already exist: {toString(overlapping_cols)}.",
        call = current_env()
      )
    }
  }

  new_data <- with_polars_errors(
    data$pivot(
      values = value_vars,
      on = names_vars,
  ...
  if (!is.null(names_glue)) {
    final_cols <- glue::glue_data(on_columns, names_glue)
    names_prefix <- NULL
  } else if (length(value_vars) > 1 && length(names_vars) == 1) {
    final_cols <- setdiff(names(new_data), data_names)
  } else {
    final_cols <- do.call(paste, c(on_columns, sep = names_sep))
  }
