...
pivot_wider.RPolarsDataFrame <- function(
  ...
  if (length(value_vars) == 0) {
    cli_abort("Must select at least one variable in `values_from`.")
  }

  if (length(names_vars) == 0) {
    cli_abort("Must select at least one variable in `names_from`.")
  }
  ...
  if (!is.null(names_prefix)) {
    if (length(names_prefix) > 1) {
      cli_abort("`names_prefix` must be of length 1.")
    }
